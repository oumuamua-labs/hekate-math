...
| `X² + X + tau_tower(m)` irreducible over GF(2^m), norm form anisotropic (carries `axioms_t.rs::norm_nonzero`, the tower-inverse field fact)                                                                                                                                                                                                                                                                                  | `build/main.rs`, `verus/axioms_t.rs`                 | `verify_norm_anisotropy`: `Tr_{GF(2^m)/GF(2)}(EXTENSION_TAU) = 1` for `m ∈ {8,16,32,64,128}` on each `cargo build`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| Production `BlockN::mul` matches its Verus twin `verus/tower/blockN.rs::mulN_k` wherever Karatsuba is compiled (16/32/256; 64 and 128 only without the `mul_iso` dispatch), τ factored through `BlockN::mul_tau` <-> `mul_tauN`. Under `pmull`, `Block64::mul` and (with `table-math`) `Block128::mul` compute φ⁻¹(φ(a)·φ(b)) via `mul_iso_N` instead                                                                        | `src/towers/*.rs` <-> `verus/tower/block*.rs`        | Transcription seam; drift caught by review + `tests/tower_oracle.rs` in release-mode CI (exhaustive at 8/16 bits, 2–20M randomized cases at 32–256)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `#[repr(transparent)]` layout: `Flat<F>` ≡ `F`, `Bit` ≡ `u8`, packed slices                                                                                                                                                                                                                                                                                                                                                  | `hardware.rs`, `packable.rs`, `bit.rs`               | Rust `repr(transparent)` guarantee                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Production `AdditiveFft` (`new` from the lift on, both scalar level-loop transforms — `fwd_levels_exec == fwd_spec`, `inv_levels_exec == inv_spec`, the sequential `blocks_serial` pass core) matches its Verus twin `verus/fft.rs::FftTwin`, which models `Flat<F>` as `u128` (k = 128), pins `usize` to 64 bits, uses `u64` for the constructor's `bits`, and gates length with `Result<(), ()>`                           | `src/fft/additive.rs` <-> `verus/fft.rs`             | Path B twin; drift caught by review + `tests/fft.rs` (round-trip, Horner differential, coset variants) in CI. The packed transforms share the scalar index shape verbatim; their element ops land on the NEON row and the lane-wise packed-vs-scalar differential (`tests/fft.rs::additive_fft_lanewise_*`) checks them on real silicon                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| rayon `par_chunks_exact_mut` / `split_at_mut` in `additive.rs::pass` (the three parallel branches) partition the same disjoint butterfly set that `blocks_serial` visits sequentially                                                                                                                                                                                                                                        | `src/fft/additive.rs::pass` <-> `verus/fft.rs`       | The twin proves `blocks_serial` + per-block write confinement (`pass_fwd_exec` / `pass_inv_exec`); butterflies within a level are independent, so scheduling order cannot change the result — only this partition contract is trusted. The lane-wise packed-vs-scalar differential runs the real dispatch above `PARALLEL_THRESHOLD_BYTES` in `tests/fft.rs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| The Cantor chain fed to `AdditiveFft::new` satisfies `fwd_semantics`' hypotheses: flat `beta[0] = 1`, `sigma_flat(beta[j]) = beta[j-1]`, and `solve_quadratic` returns a root of `x^2 + x = c` iff `Tr(c) = 0`                                                                                                                                                                                                               | `build/main.rs::write_algebra_extras_{16,32,64,128}` | Build-time checks on the real constants, each `cargo build`. At 16, exhaustive: tower chain + flat-basis chain (`sigma_flat` descent, `beta_0 = 1`); Artin–Schreier solvability round-trip on all 2^15 trace-zero inputs. At 32/64/128, basis-complete: `solve ∘ L~ = id` and `L~ ∘ solve = id` on every basis vector (GF(2)-linear maps agreeing on a basis agree everywhere), `Tr(d) = 1`, 64 fixed-seed root-pair round-trips, and the full-depth tower chain (rank N, sigma descent, `s_i(beta_i) = 1`, trace pattern); the flat-basis form of the wide chains rides the T4 iso rows (`phi` ring homomorphism), and `tests/fft.rs` round-trips the real flat schedules at 32/64/128. The constructor twin proves the twiddle schedule is the point map of the lift (`tw_sum_is_point`), so only the lift values themselves rest on this row |
...
No `admit()` remains in `gf_model.rs` (113 verified, 0 errors). The former three:
...
the macro-generated `square` split at every width
(`square{16,32,64,128}_twin_correct`, via `bridge{16,32,64,128}` and
`schoolbook{16,32,64}_comm`), the default frobenius loop
(`frobenius16_semantics`), and the default trace loop (`trace_iter16_reflect`,
`trace16_idempotent`). `solve_quadratic`'s value path is the proven `map_ct`
fold at every width (`map_ct_{16,32,64,128_split}`, the CT-kernel row above);
its basis constants land on the `write_algebra_extras_*` row. Trace membership
in `{0, 1}` additionally needs zero-divisor freedom, which stays with the
exhaustive 16-bit build check in `write_algebra_extras_16`; the wide mask
traces are pinned to the squaring-chain definition by `tests/algebra.rs`
(4096 seeded samples per width).

Additive FFT (`verus/fft.rs`), all relative to the `mul_flat` row: index
safety and panic-freedom of the level-pass loops and the constructor's
`trailing_zeros` loop are proven exec twins; the twins' functional ensures
tie them to `fwd_spec` / `inv_spec` / `tw_sum` (the level schedule refines
the recursive spec through the strided-gather invariant). `roundtrip` proves
...
isolated with `norm_nonzero` and `frobenius_order_gen` in
`verus/axioms_t.rs`, the tree's only model-axiom file. The one
`external_body` item outside it is a seam row registered above:
the `fft.rs` `mul_flat` stub.
...
`mul_flat_128` (limb schoolbook via `schoolbook_clmul`; the Karatsuba
form and its mid-term cancellation remain proven as `karatsuba_clmul`
for the packed byte kernels). Packed kernels are proven per lane:
...
and the 32/128 lane loops plus the packed-64 lane pipeline
(per-lane values identical to the scalar kernel). The conversion kernels
