...
`verus/axioms_t.rs`, the tree's only model-axiom file. The three
`external_body` items outside it are seam rows registered above:
the `matrix.rs` `Vec` contracts and the `fft.rs` `mul_flat` stub.
