...
impl PackableField for Block32 {
    ...
    fn pack(chunk: &[Self]) -> Self::Packed {
        ...
        unsafe {
            let mut arr = core::mem::MaybeUninit::<[Self; PACKED_WIDTH_32]>::uninit();
            let dst = arr.as_mut_ptr() as *mut Self;
            core::ptr::copy_nonoverlapping(chunk.as_ptr(), dst, PACKED_WIDTH_32);

            PackedBlock32(arr.assume_init())
        }
    }
    ...
    fn unpack(packed: Self::Packed, output: &mut [Self]) {
        ...
        unsafe {
            core::ptr::copy_nonoverlapping(packed.0.as_ptr(), output.as_mut_ptr(), PACKED_WIDTH_32);
        }
    }
}
...
impl HardwareField for Block32 {
    ...
    fn tower_bit_from_hardware(self, bit_idx: usize) -> u8 {
        debug_assert!(bit_idx < 32, "bit index out of bounds for Block32");

        let mask = unsafe { *constants::FLAT_TO_TOWER_BIT_MASKS_32.get_unchecked(bit_idx) };
